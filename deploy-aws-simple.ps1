# Akazuba Florist AWS Deployment Script (PowerShell) - Simplified Version
param(
...
$TemplateContent | Out-File -FilePath "cloudformation-template.yaml" -Encoding UTF8
...
try {
    aws cloudformation create-stack --stack-name "${ProjectName}-infrastructure" --template-body file://cloudformation-template.yaml --capabilities CAPABILITY_IAM --region $Region
    Write-Host "⏳ Waiting for CloudFormation stack to complete..." -ForegroundColor Blue
    aws cloudformation wait stack-create-complete --stack-name "${ProjectName}-infrastructure" --region $Region
...
try {
    $EC2_IP = aws cloudformation describe-stacks --stack-name "${ProjectName}-infrastructure" --query 'Stacks[0].Outputs[?OutputKey==`EC2PublicIP`].OutputValue' --output text --region $Region
    $RDS_ENDPOINT = aws cloudformation describe-stacks --stack-name "${ProjectName}-infrastructure" --query 'Stacks[0].Outputs[?OutputKey==`RDSEndpoint`].OutputValue' --output text --region $Region
