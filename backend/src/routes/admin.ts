...
router.get('/notifications', async (req, res) => {
  try {
    ...
    const [recentOrders, lowStockProducts, recentUsers] = await Promise.all([
      prisma.order.findMany({
        take: 5,
        orderBy: { createdAt: 'desc' },
        include: {
    ...
    const notifications = [
      ...
      ...recentOrders.map((order, index) => ({
        ...
        type: 'order',
        title: 'New Order Received',
        message: `Order #${order.orderNumber || `ORD-${order.id.slice(-6)}`} from ${order.user ? `${order.user.firstName} ${order.user.lastName}` : 'Guest User'} for ${new Intl.NumberFormat('en-RW', { style: 'currency', currency: 'RWF' }).format(Number(order.totalAmount))}`,
        timestamp: order.createdAt,
      ...
      ...lowStockProducts.map((product, index) => ({
        ...
        message: `${product.name} is running low (${product.stockQuantity} units left)`,
        timestamp: product.updatedAt,
        read: false,
        priority: product.stockQuantity <= 5 ? 'urgent' : 'high',
        actionRequired: true,
        actionUrl: `/admin/products/${product.id}`,
        metadata: { productId: product.id, stockQuantity: product.stockQuantity }
      })),
