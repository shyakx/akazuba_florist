...
import { 
  Package, Save, X, Upload, Image as ImageIcon, 
  DollarSign, Tag, FileText, AlertTriangle, Edit, ArrowLeft
} from 'lucide-react'
...
const AdminEditProductPage = () => {
  ...
  const loadProduct = async () => {
    try {
      ...
      // Get data from backend API instead of local storage
      ...
      const productData = data.data.products.find((p: any) => p.id === productId)
      ...
      if (productData) {
        ...
        setFormData({
          ...
          categoryName: productData.categoryName || productData.category?.name || '',
          stockQuantity: productData.stockQuantity?.toString() || '',
          minStockAlert: productData.minStockAlert?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const baseURL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'
      const response = await fetch(`${baseURL}/api/v1/admin/products/${product.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 text-lg">Update product information and settings</p>
            </div>
